...
def p_declaration_keyword(p):
	'''declaration_keyword : KEYWORD_VAR
										| KEYWORD_VAL '''
	child1 = create_leaf("KEYWORD VAR/VAL", p[1])
...
def p_do_while_statement(p):
	...
	child5 = create_leaf("TERMINATOR", p[7])
...
def p_for_logic(p):
		...
		if len(p)==2:
			...
			child1 = create_leaf("TERMINATOR",p[2])
...
def p_empty_statement(p):
				...
				child1 = create_leaf("TERMINATOR", p[1])
...
def p_return_statement(p):
				...
				child2 = create_leaf("TERMINATOR", p[3])
